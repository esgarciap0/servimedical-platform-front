import type { AphForm } from '../../types/aph'

export const initialForm: AphForm = {
  movil: '',
  placa: '',
  esAtencionInicialPacienteRemitidoOControl: '1',
  traslado: 'PRIMARIO',
  codigoHabilitacionRecibeTransportePrimario: '',
  direccionOrigenTransportePrimario: '',
  direccionDestinoTransportePrimario: '',
  tipoTraslado: 'BASICO',
  prioridad: '1',
  fechaAccidente: '',
  horaAccidente: '',
  naturalezaEvento: '',
  descripcionOtroEvento: '',
  condicionVictima: '',
  estadoAseguramiento: '',
  placaVehiculo: '',
  tipoVehiculo: '',
  codigoAseguradora: '',
  numeroPolizaSoat: '',
  fechaInicioVigencia: '',
  fechaFinVigencia: '',
  numeroRadicadoSiras: '',
  tipoDocumentoPropietario: '',
  numeroDocumentoPropietario: '',
  lugarOcurrencia: '',
  zonaOrigen: '',
  codigoMunicipioOcurrencia: '',
  departamentoOrigen: '',
  municipioOrigen: '',
  documento: '',
  tipoDocumento: 'CC',
  primerApellido: '',
  segundoApellido: '',
  primerNombre: '',
  segundoNombre: '',
  estadoCivil: '',
  ocupacion: '',
  sexo: '',
  fechaNacimiento: '',
  edad: '',
  celular: '',
  telefono: '',
  tipoPoblacion: '',
  acompanante: '',
  celularAcompanante: '',
  avisarA: '',
  parentesco: '',
  numeroParaAvisar: '',
  numeroParaAvisar2: '',
  direccion: '',
  codigoMunicipioResidencia: '',
  zonaPaciente: '',
  departamento: '',
  ciudad: '',
  alergia: '',
  patologicos: '',
  medicacion: '',
  liquidos: '',
  aseguradora: '',
  poliza: '',
  planBeneficios: 'SOAT',
  fechaAceptacion: '',
  horaAceptacion: '',
  horaLlegada: '',
  tipoServicioTransporte: '',
  transportadoA: '',
  codigoHabilitacion: '',
  codigoHabilitacionPrestadorRecibe: '',
  departamentoTraslado: '',
  ciudadTransporte: '',
  estadoPaciente: 'VIVO',
  causaExterna: '',
  presion: '',
  frecuenciaCardiaca: '',
  frecuenciaRespiratoria: '',
  temperatura: '',
  ro: '1',
  rv: '1',
  rm: '1',
  hallazgos: '',
  diagnosticos: '',
  primerNombrePropietario: '',
  segundoNombrePropietario: '',
  primerApellidoPropietario: '',
  segundoApellidoPropietario: '',
  direccionResidenciaPropietario: '',
  telefonoResidenciaPropietario: '',
  codigoMunicipioResidenciaPropietario: '',
  tipoDocumentoConductorVehiculo: '',
  numeroDocumentoConductorVehiculo: '',
  primerNombreConductorVehiculo: '',
  segundoNombreConductorVehiculo: '',
  primerApellidoConductorVehiculo: '',
  segundoApellidoConductorVehiculo: '',
  codigoMunicipioResidenciaConductorVehiculo: '',
  direccionResidenciaConductorVehiculo: '',
  telefonoResidenciaConductorVehiculo: '',
  materiales: '',
  conductor: '',
  documentoConductor: '',
  paramedico: '',
  documentoParamedico: '',
  medico: '',
  tipoDocumentoProfesionalRecibe: '',
  documentoMedico: '',
}
